...
message * msg_create_args(const char *tname, const char *sig, ...) {
    const message_type *type = mt_find(tname);
    message *msg;
    va_list vargs;
    ...
    while (sig) {
        ...
        object * obj = va_arg(vargs, object *);
        assert(obj && obj->type);
        if (end) {
            bytes = end - sig;
        }
        ...
        for (i = 0; i != type->nparameters; ++i) {
            if (strncmp(type->pnames[i], sig, bytes) == 0) {
                break;
            }
        }
        assert(i != type->nparameters); // TODO: error handling
        strlcpy(token, sig, bytes);
        args[numargs].data = obj;
        args[numargs].name = type->pnames[i];
        sig = end;
        ++numargs;
    }
    va_end(vargs);
    ...
    msg->type = type;
    msg->arguments = (argument *)malloc(sizeof(argument) * numargs);
    memcpy(msg->arguments, args, sizeof(argument) * numargs);
    msg->numargs = numargs;
    msg->refcount = 1;
    return msg;
}

object_type * find_type(const char *name) {
...
message *msg_create(const struct message_type *mtype, variant args[])
{
    ...
    msg->type = mtype;
    msg->arguments = (argument *)malloc(sizeof(argument) * mtype->nparameters);
    msg->refcount = 1;
    for (i = 0; i != mtype->nparameters; ++i) {
        object * obj = (object *)malloc(sizeof(object));
        obj->type = find_type(mtype->types[i]->name);
        obj->data = args[i];
        msg->arguments[i].data = obj;
        msg->arguments[i].name = mtype->pnames[i];
    }
    if (msg_log_create)
        msg_log_create(msg);
    return msg;
...
void msg_free(message * msg)
{
    ...
    for (i = 0; i != msg->type->nparameters; ++i) {
        object * obj = msg->arguments[i].data;
        obj->type->release(obj->data);
        free(obj);
    }
    free(msg->arguments);
    free(msg);
